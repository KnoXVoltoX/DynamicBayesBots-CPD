...
std::vector<double> get_observation_means(std::vector<Kilosim::Robot *> &robots)
{
    // Get each individual robot's cpd
    std::vector<double> cpds(robots.size());
    for (int i = 0; i < robots.size(); i++)
    {
        Kilosim::MyKilobot *kb = (Kilosim::MyKilobot *)robots[i];
        cpds[i] = kb->log_noCPD;
...
std::vector<double> raw_observe(std::vector<Kilosim::Robot *> &robots)
{
    // Get each individual robot's raw color observation
    std::vector<double> raw_obs(robots.size());
    for (int i = 0; i < robots.size(); i++)
    {
        Kilosim::MyKilobot *kb = (Kilosim::MyKilobot *)robots[i];
        raw_obs[i] = kb->log_C;
...
int main(int argc, char *argv[])
{
    ...
    const std::string filename = config.get("log_filename");
    ...
    for (uint compare_ind = 0; compare_ind < num_compare_vals; compare_ind++)
    {
        ...
        // std::string log_filename = log_dir + "log_" + trial_dur + "s_" +"pc=0."+cred_thresh+".h5";//+ compare_param + '=' + compare_val_str + ".h5";
        std::string log_filename = log_dir + filename + "_" + trial_dur + "s_" +"pc=0."+cred_thresh+".h5";
        ...
        for (uint trial = start_trial; trial < (num_trials + start_trial); trial++)
        {
            ...
            printf("\n\n");
            printf("-------------------------------------------------------\n");
            std::cout << "    TRIAL " << trial << " " << filename << std::endl;//compare_param << " = " << compare_val << std::endl;
            printf("-------------------------------------------------------\n");
            ...
            logger.add_aggregator("raw_observation", raw_observe);
            logger.add_aggregator("observation_mean", get_observation_means);
    ...
    printf("Simulations of successfully completed\n\n");
    std::cerr << "Trial = " << filename << std::endl;
