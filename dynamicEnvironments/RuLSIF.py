...
"""[1:-1] % dict(method=self.method, kernel_info=self.kernel_info, alpha=self.alpha, theta=self.theta, lambda_=self.lambda_, alpha_PE=self.alpha_PE, alpha_KL=self.alpha_KL)
...
class KernelInfo:
  ...
  Centers: %(centers)s
"""[1:-1] % dict(kernel_type=self.kernel_type, kernel_num=self.kernel_num, sigma=self.sigma, centers=self.centers)
...
def plot_posterior(T, data, cps, dens, window_size):
    ...
    for cp in cps:
        ...
        ax2.axvline(cp/window_size, c='red', ls='dotted')
...
if __name__ == '__main__':
    ...
    for i in range(2*window_size, len(data)):#,window_size):
        ...
        if i%window_size == 0:
            ...
            sum = 0
    print("dens",dens)
    cps = [dens.index(i)*window_size for i in dens if i >= np.max(dens)-np.max(dens)*thresh]
    print("cps",np.asarray(cps)*10)
    plot_posterior(T, data, cps, dens, window_size)
